...
def _p_m(k_z, n2, l, s2h):
    ...
    """
    p = n2[l+1]*k_z[l] + n2[l]*k_z[l+1]
    m = n2[l+1]*k_z[l] - n2[l]*k_z[l+1]
    rp = cmath.exp(-_sq(k_z[l] - k_z[l+1]) * s2h[l])
    rm = cmath.exp(-_sq(k_z[l] + k_z[l+1]) * s2h[l])
    o = 2 * n2[l+1]*k_z[l]
...
@jit
def _reflec_and_trans_inner(k2n2, k2, n2, theta, thick, s2h):
...
def reflec_and_trans(n, lam, thetas, thick, rough):
    ...
    for i in range(n_theta):
        r, t = _reflec_and_trans_inner(k2n2, k2, n2, thetas[i], thick, s2h)
        rs[i] = r
        ts[i] = t
    return rs, ts
...
@jit
def _fields_inner(k2n2, k2, n2, theta, thick, s2h, mm12, mm22, rs, ts, kt, N):
    # wavevectors in the different layers
    k2_x = k2 * _sq(math.cos(theta))  # k_x is conserved due to snell's law
    k_z = -np.sqrt(k2n2 - k2_x)  # k_z is different for each layer.
...
def fields(n, lam, thetas, thick, rough):
    ...
    for kt, theta in enumerate(thetas):
        _fields_inner(k2n2, k2, n2, theta, thick, s2h, mm12, mm22, rs, ts, kt, N)
    return rs, ts
...
def fields_parallel(n, lam, thetas, thick, rough):
    ...
    for kt in prange(T):
        ...
        mm12 = np.empty(N + 1, dtype=np.complex128)
        mm22 = np.empty(N + 1, dtype=np.complex128)
        _fields_inner(k2n2, k2, n2, thetas[kt], thick, s2h, mm12, mm22, rs, ts, kt, N)
    return rs, ts
...
def fields_at_positions(n, lam, thetas, thick, rough, evaluation_positions):
    ...
    for kt in range(T):
        _fields_positions_inner(thick, s2h, kt, rs, ts, mm12, mm22, N, k_z, n2)
